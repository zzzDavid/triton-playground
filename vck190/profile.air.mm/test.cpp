...
int main(int argc, char *argv[]) {
  ...
  u32 pc0_times[1]; // tracks timer values
  u32 pfc = 1; // performance counter number
  ...
  XAie_EventPCEnable(&(xaie->DevInst), XAie_TileLoc(col, row), 0, 0);
  // Trigger off done (0x088) of an AIE program
  XAie_EventPCEnable(&(xaie->DevInst), XAie_TileLoc(col, row), 1, 240);

  // Define custom EventMonitor class to track event triggers for program
  // counter
  EventMonitor pc0(xaie, col, row, pfc, XAIE_EVENT_PC_0_CORE, XAIE_EVENT_PC_1_CORE,
